...
async def stage1_collect_responses(user_query: str) -> List[Dict[str, Any]]:
    ...
    """
    # Members are raw API calls with no harness. Assistant-tuned models
    # (notably Claude via Meridian) otherwise roleplay their product tooling —
    # memory writes, tool-call XML — instead of answering (see runs #3 and the
    # 2026-07-21 shopchat run, where Opus burned its seat on a fake memory op).
    member_system = (
        "You are one member of an advisory council. You have NO tools, NO memory, "
        "and NO ability to execute anything - do not emit tool calls, XML tags, or "
        "system reminders. Reply with plain-text substantive analysis only."
    )
    messages = [
...
Title:"""
    ...
    # A small, fast model is plenty for naming a conversation.
